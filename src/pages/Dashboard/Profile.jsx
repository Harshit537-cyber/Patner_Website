...
const Profile = () => {
  ...
  const displayProfile = {
    ...
    bio: profile.professional.bio || 'Vedic astrologer specializing in career and relationship guidance.',
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      <ProfileHeader
        ...
        specialization={profile.professional.specialization || 'Vedic Astrology'}
        rating={4.8}
      />
      {editing ? (
        <EditProfile
          profile={displayProfile}
          onSave={(data) => { updateSection('personal', data); setEditing(false); }}
        />
      ) : (
        <div>
          ...
          <button
            ...
            style={{ marginTop: 12, background: 'none', border: 'none', color: 'var(--color-primary)', fontWeight: 600, cursor: 'pointer', fontSize: '0.88rem' }}
          >
            Edit profile
          </button>
        </div>
      )}
      <ConsultationRates rates={rates} onSave={(data) => updateSection('professional', data)} />
    </div>
  );
};
